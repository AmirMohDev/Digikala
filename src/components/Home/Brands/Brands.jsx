    ...
    <div className="w-full py-4 relative px-4 md:px-6 lg:px-8 max-w-[1300px] mx-auto border-2 border-gray-200 rounded-2xl">
      ...
      >
        {brands?.map((brand, index) => {
          // Create a stable key using the brand's properties
          const brandKey = brand.id
            ? `brand-${brand.id}`
            : `brand-${index}-${brand.name?.replace(/\s+/g, "-") || "unknown"}`;

          return (
            <SwiperSlide
              key={brandKey}
              className="w-[60px] sm:w-[70px] md:w-[80px] lg:w-[90px] xl:w-[100px] border-l border-gray-200 border-h-1"
            >
